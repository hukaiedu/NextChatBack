 ...
 *    - 存在聊天输入区 `textarea` → 已登录(Gemini 登录后必渲染)
 *    - 无 textarea 且存在指向 accounts.google.com 的链接 → 未登录
 *      (未登录时页面停留在本站但显示登录入口,不会总是跳转 accounts)
 *    - 两者都无(SPA 渲染未完成等)→ 保守判未登录
 *
 * 注意:已登录页面长得完全不同 — 真实 DOM 里输入框是 <textarea>,
 * 不是 contenteditable,且已登录页仍存在 accounts.google.com 链接
 * (头像菜单的 SignOutOptions),所以 accounts 链接不能作未登录的单独判据,
 * 必须先确认"无输入框"再说。
 */
export class GeminiSessionChecker {
  private readonly signInSelector =
    'a[href*="accounts.google.com"], a[href*="ServiceLogin"]';
  private readonly composerSelector = "textarea";
  ...
  async checkLoggedIn(page: BrowserPageHandle): Promise<boolean> {
    ...
    try {
      // 正判据:聊天输入区 textarea(登录后必有)
      const composers = await page.countElements(this.composerSelector);
      if (composers > 0) {
        return true;
      }
      // 无输入区:存在 accounts 链接 → 页面在显示登录入口
      const signInLinks = await page.countElements(this.signInSelector);
      if (signInLinks > 0) {
        return false;
      }
