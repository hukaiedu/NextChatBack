...
describe("Provider API", () => {
  ...
  it("GET /api/admin/provider/status:初始 STOPPED(不启动浏览器)", async () => {
    const res = await api("/api/admin/provider/status");
  ...
  it("POST /api/admin/provider/open:未登录 → LOGIN_REQUIRED", async () => {
    driver.redirectToLogin = true;

    const res = await api("/api/admin/provider/open", { method: "POST" });
    expect(res.status).toBe(200);
    const body = (await res.json()) as ProviderStatusBody;
  ...
  it("POST /api/admin/provider/open:已登录 → READY,重复 open 不二次启动", async () => {
    const first = await api("/api/admin/provider/open", { method: "POST" });
    expect(((await first.json()) as ProviderStatusBody).data.status).toBe("READY");
    expect(driver.launchCount).toBe(1);

    const second = await api("/api/admin/provider/open", { method: "POST" });
    expect(((await second.json()) as ProviderStatusBody).data.status).toBe("READY");
    expect(driver.launchCount).toBe(1);
  });

  it("POST /api/admin/provider/open:Profile 被占用 → 500 PROVIDER_PROFILE_IN_USE,状态 ERROR", async () => {
    ...
    const status = await api("/api/admin/provider/status");
    expect(((await status.json()) as ProviderStatusBody).data.status).toBe("ERROR");
  });

  it("POST /api/admin/provider/open:启动失败 → 500 PROVIDER_BROWSER_START_FAILED", async () => {
  ...
  it("POST /api/admin/provider/restart:关闭 Context → 同一 Profile 重启 → READY", async () => {
    await api("/api/admin/provider/open", { method: "POST" });
    expect(driver.launchCount).toBe(1);

    const res = await api("/api/admin/provider/restart", { method: "POST" });
    expect(res.status).toBe(200);
